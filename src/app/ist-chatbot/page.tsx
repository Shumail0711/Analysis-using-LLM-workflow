...
export default async function IstChatbotPage() {
  const [istChat] = await db.select({ id: chats.id }).from(chats).where(eq(chats.name, 'ist'));

  if (!istChat) {
    notFound();
  }

  const chatId = istChat.id;
      ...
      <div className='sticky top-0 z-10 flex h-14 items-center justify-between border-b bg-background px-4 sm:hidden'>
        ...
        <Sheet>
          ...
          <SheetContent side='left' className='space-y-2'>
            <ChatSidebar chat={chat} chatFiles={chatFiles} settings={settings} hideFiles />
          </SheetContent>
        </Sheet>
      </div>
